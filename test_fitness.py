...
def calc_score(egraph, root, imgs, masks, params):
    global count
...
def test_opt():
    ...
    egraph, eids = get_fst_rnd_egraph(100)
    root = eids[max(eids.keys())][0]
    ...
    score = calc_score(egraph, root, imgs, masks, params)
    ...
    # plot the scores evolution for each param 
    import matplotlib.pyplot as plt 
    for i in range(len(scs)):
        plt.plot(np.arange(0,256,1), scs[i], '.')
        plt.title(f"Param {i} score evolution")
